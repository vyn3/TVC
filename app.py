...
def _update_imu(payload):
    if not isinstance(payload, dict):
        raise ValueError
    imu = state["imu"]

    def update_axis_block(block_name, axes, limits):
        ...
        for ax in axes:
            if ax in block:
                val = _coerce_float(block[ax])
                lo, hi = limits
                imu[block_name][ax] = _clamp(val, lo, hi)

    update_axis_block("angle_deg", ("x", "y"), (-180.0, 180.0))
    update_axis_block("gyro_dps", ("x", "y", "z"), (-2000.0, 2000.0))
    update_axis_block("accel_g", ("x", "y", "z"), (-16.0, 16.0))

    if "temp_c" in payload:
        val = _coerce_float(payload["temp_c"])
        imu["temp_c"] = _clamp(val, -50.0, 125.0)
...
def api_imu_update():
    ...
    try:
        ...
        return (
            jsonify(ok=False, error="IMU invalide", msg="IMU invalide"),
            400,
        )
    return jsonify(ok=True, msg="IMU mise à jour", imu=state["imu"])
